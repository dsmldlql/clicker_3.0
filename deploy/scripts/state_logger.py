...
class StateLogger:
  ...
  def __init__(self, bot_id: int, site: str, scenario: str, log_dir: str):
    ...
    # Путь к файлу логов — bot_id + дата в имени файла
    os.makedirs(log_dir, exist_ok=True)
    self.state_log_path = os.path.join(log_dir, f"state_timeline_bot_{bot_id}_{site}_{scenario}_{datetime.now().strftime('%Y%m%d')}.jsonl")
